...
class HSTB(nn.Module):
    r""" Hybrid Swin Transformer Block.
...
class RAB(nn.Module):
    ...
    def forward(self, x, x_size, mask=None):
        ...
        x1 = self.attns[0](qkv[:, :, :, :c // 2], x_size, mask)
        x2 = self.attns[1](qkv[:, :, :, c // 2:], x_size, mask)
...
class FourierUnit(nn.Module):
    def __init__(self, dim, fft_norm='ortho'):
        ...
        self.conv_layer = nn.Conv2d(dim * 2, dim * 2, 1, 1, 0)
...
class FFGB(nn.Module):
    """
    Fast Fourier Global Block
    """
    def __init__(self, dim, last_conv=False):
        # bn_layer not used
        super(FFGB, self).__init__()
...
class GMIB(nn.Module):
    r"""  Global and Multi-scale Information Block
    ...
    def __init__(self,
        ...
        self.attns = RAB(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,split_size=split_size,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        self.fourier = FFGB(dim)
    ...
    def forward(self, x, x_size):
        ...
        f = self.fourier(x.permute(0, 3, 1, 2))
        ...
        attn_windows = self.attns(x_windows, x_size)

        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, c)
        shifted_x = window_reverse(attn_windows, self.window_size, h, w)  # B H' W' C
        if self.shift_size > 0:
            attn = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
        else:
            attn = shifted_x

        #x = self.fusion()
        x = torch.cat([attn.permute(0, 3, 1, 2), f], dim=1)
        x = self.fusion(x)
...
class RHAG(nn.Module):
    ...
    def forward(self, x, x_size, params):
        ...
        #print(self.patch_embed(self.patch_unembed(x,x_size)).shape)
...
class SSRT(nn.Module):
    r""" Spacecraft image Super Resolution Transformer
    Args:
    ...
    def __init__(self,
        ...
        #self.ra_windows_sizes = ra_window_sizes
